...
from src.audio import WaveReader, Reader
from src.metric import si_snr, permute_si_snr
...
class SpeakersReader(object):
    def __init__(self, scps):
        ...
        if len(split_scps) == 1:
            raise RuntimeError("Construct SpeakersReader need more than one script, got {}".format(scps))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Command to compute SI-SDR, as metric of the separation quality",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--sep_scp",
                        type=str,
                        help="Separated speech scripts, waiting for measure (support multi-speaker,"
                             "egs: spk1.scp,spk2.scp)")
